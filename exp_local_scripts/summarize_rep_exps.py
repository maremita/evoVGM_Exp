...
if __name__ == '__main__':
            ...
            for len_aln in len_alns:
                for evomodel in model_types:
                    ...
                    if not os.path.isfile(results_file):
                        ...
                        val_results = [result["val_results"] for result in rep_results]
                        ...
                        title = exp_name
                        if False:
                            if verbose: print("Plotting {}..".format(exp_name))
                            plt_elbo_ll_kl_rep_figure(
                                    the_scores,
                                    output_exp+"/{}_exp{}_fig".format(exp_name, report_n_epochs),
                                    print_xtick_every=200,
                                    usetex=False,
                                    y_limits=[-100, 0],
                                    title=title,
    ...
    if Summarize_model_wise:
        ...
        #col_index = pd.MultiIndex.from_product([len_alns, ['Real', 'Mean', 'Variance']])
        col_index = pd.MultiIndex.from_product([len_alns, ['Real', 'Mean', 'STD']])

        branch_dict = dict()
        col_index_b = pd.MultiIndex.from_product([len_alns, ['dist', 'corr', 'pval']])

        rates_dict = dict()
        freqs_dict = dict()
        kappa_dict = dict()
        col_index_k = pd.MultiIndex.from_product([len_alns, ['kappa']])
        ...
        for data_model, rates, freqs in data_types:
            for evomodel in model_types:
                ...
                df_l = pd.DataFrame("", index=row_index, columns=col_index)
                df_b = pd.DataFrame("", index=row_index, columns=col_index_b)
                if evomodel == "gtr":
                    df_r = pd.DataFrame("", index=row_index, columns=col_index_b)
                    df_f = pd.DataFrame("", index=row_index, columns=col_index_b)
                elif evomodel == "k80":
                    df_k = pd.DataFrame("", index=row_index, columns=col_index_k)
                ...
                for nb_seqs, branch_lens in branches:
                    for len_aln in len_alns:
                        ...
                        # Log likelihood
                        # ##############
                        df_l[len_aln, "Real"].loc[nb_seqs] = val_dict[exp_name+"_logl"].item()
                        val_results = val_dict[exp_name] # a list of dictionaries
                        logls = np.array([measures["logl"].item() for measures in val_results])

                        # val scores of last iteration from fit
                        #fit_results = fit_dict[exp_name] # a list of dictionaries
                        #logls = fit_results[:, 4, -1]
                        #print(logls.shape)

                        df_l[len_aln, "Mean"].loc[nb_seqs] = logls.mean()
                        #df_l[len_aln, "Variance"].loc[nb_seqs] = logls.var()
                        df_l[len_aln, "STD"].loc[nb_seqs] = logls.std()

                        # Branches
                        # ########
                        bls = np.array([measures["b"].numpy() for measures in val_results])
                        bls = bls.mean(0).flatten()
                        df_b[len_aln, "dist"].loc[nb_seqs] = pdist(
                                np.vstack((branch_lens, bls)),'euclidean')[0]
                        ...
                        # #############
                        if evomodel == "gtr":
                            rs = np.array([measures["r"].numpy() for measures in val_results])
                            rs = rs.mean(0).flatten()

                            fs = np.array([measures["f"].numpy() for measures in val_results])
                            fs = fs.mean(0).flatten()

                            df_r[len_aln, "dist"].loc[nb_seqs] = pdist(
                                    np.vstack((rates, rs)),'euclidean')[0]

                            df_f[len_aln, "dist"].loc[nb_seqs] = pdist(
                                    np.vstack((freqs, fs)),'euclidean')[0]

                            if data_model != "jc69":
                                corr_r = pearsonr(rates, rs)
                                df_r[len_aln, "corr"].loc[nb_seqs] = corr_r[0]
                                df_r[len_aln, "pval"].loc[nb_seqs] = corr_r[1]

                                corr_f = pearsonr(freqs, fs)
                                df_f[len_aln, "corr"].loc[nb_seqs] = corr_f[0]
                                df_f[len_aln, "pval"].loc[nb_seqs] = corr_f[1]

                        elif evomodel == "k80":
                            ks = np.array([measures["k"].numpy() for measures in val_results])
        ...
        #write_dict_df(logl_dict, output_dir+"logl_model_wise_var.txt")
        write_dict_df(logl_dict, output_dir+"logl_model_wise_std.txt")
        #write_dict_df(branch_dict, output_dir+"branches.txt")
        #write_dict_df(rates_dict, output_dir+"rates.txt")
        #write_dict_df(freqs_dict, output_dir+"freqs.txt")
        #write_dict_df(kappa_dict, output_dir+"kappa.txt")
        
    Summarize_data_wise = True
    if Summarize_data_wise:
        # outputdir 
        output_sum = os.path.join(output_dir, "summarize_nb_len")
        makedirs(output_sum, mode=0o700, exist_ok=True)

        evo_data = dict()
        row_index = ['Real'] + model_types
        #col_index = pd.MultiIndex.from_product([[d for d,_,_ in data_types], ['Mean', 'Variance']])
        col_index = pd.MultiIndex.from_product([[d for d,_,_ in data_types], ['Mean', 'STD']])

        for nb_seqs, branch_lens in branches:
            for len_aln in len_alns:
                # create the dataframe
                code = "n{}_l{}".format(nb_seqs, len_aln)
                df_l = pd.DataFrame("", index=row_index, columns=col_index)
                ...
                for data_model, rates, freqs in data_types:
                    ...
                    for evomodel in model_types:
                        ...
                        df_l[data_model, "Mean"].loc["Real"] = val_dict[exp_name+"_logl"].item()
                        ...
                        df_l[data_model, "Mean"].loc[evomodel] = logls.mean()
                        #df_l[data_model, "Variance"].loc[evomodel] = logls.var()
                        df_l[data_model, "STD"].loc[evomodel] = logls.std()
                    ...
                    # print(fit_elbos[data_model].shape) # (nb_models, nb_rep, nb_epoch)

                print(code)
                if False:
                    out_file = output_sum+"/{}_".format(code) 
        ...
        #write_dict_df(evo_data, output_sum+"/logl_data_wise_var.txt")
        write_dict_df(evo_data, output_sum+"/logl_data_wise_std.txt")
